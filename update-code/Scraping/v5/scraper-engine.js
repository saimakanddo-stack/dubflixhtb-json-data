...
class ScraperEngine {
    constructor(config = {}) {
        ...
        this.proxyFailures = new Map(); // name -> consecutive failures
        this.maxConsecutiveFailures = 3; // Skip proxy after 3 failures in a row
    }
    ...
    async fetchHTML(url, useCache = true, signal = null) {
        ...
        const allProxies = [
            ...
            { name: 'ThingProxy', url: 'https://thingproxy.freeboard.io/fetch/', type: 'raw' }
        ];

        let proxies = isTelegram ? [
            allProxies[0], // CorsProxy.io
            allProxies[1], // CodeTabs
            allProxies[2], // AllOrigins JSON
            allProxies[3], // Hiproxy
            allProxies[4], // AllOrigins Raw
            allProxies[5]  // ThingProxy
        ] : allProxies;

        // Filter out unhealthy proxies (too many failures)
        proxies = proxies.filter(p => (this.proxyFailures.get(p.name) || 0) < this.maxConsecutiveFailures);

        // If all proxies are unhealthy, reset tracking and try again
        if (proxies.length === 0) {
            if (this.debug) console.log('[ScraperEngine] All proxies unhealthy, resetting failure tracking');
            this.proxyFailures.clear();
            proxies = isTelegram ? [allProxies[0], allProxies[1], allProxies[2], allProxies[3], allProxies[4], allProxies[5]] : allProxies;
        }

        // DYNAMIC OPTIMIZATION: Move last known good proxy to front
        ...
        // Add DIRECT fetch as a first attempt or fallback if environment allows
        // On modern browsers or certain environments, direct fetch might work if target has CORS headers
        proxies.unshift({ name: 'Direct Fetch', url: '', type: 'direct' });
            ...
            try {
                ...
                try {
                    ...
                    if (proxy.type === 'direct') {
                        response = await fetch(url, { signal: fetchSignal, credentials: 'omit' });
                    } else {
                        const fetchURL = proxy.url + encodeURIComponent(url);
                        response = await fetch(fetchURL, { signal: fetchSignal, credentials: 'omit' });
                    }
                    ...
                    if (html && html.length > 0) {
                        // VALIDATION: Check if response is actually the movie site and not a bot challenge
                        ...
                        this.proxyFailures.set(proxy.name, 0);
                ...
                // Track failure (except for Direct Fetch which is expected to fail often)
                if (proxy.type !== 'direct') {
                    const currentFailures = (this.proxyFailures.get(proxy.name) || 0) + 1;
                    this.proxyFailures.set(proxy.name, currentFailures);
                }
            }
        }
    ...
    /**
     * Validate if the HTML is actual content or a block/challenge page
    ...
    isValidHTML(html) {
        ...
        // Common block/challenge strings
        const blockStrings = [
            'attention required!',
        ...
        // If it contains a block string, it might be a block page
        const hasBlockString = blockStrings.some(s => lowerHTML.includes(s));

        // Positive indicators (must contain some movie site specific markers)
        const hasTargetMarkers =
            lowerHTML.includes('movie') ||
            lowerHTML.includes('download') ||
            lowerHTML.includes('episode') ||
            lowerHTML.includes('post-') ||
            lowerHTML.includes('view');

        // If it's very short, it's probably not a full page
        if (html.length < 500 && !hasTargetMarkers) return false;

        // If it has a block string BUT doesn't have our target markers, it's definitely a block page
        if (hasBlockString && !hasTargetMarkers) return false;
     ...
     */
    /**
     * Handle one-click download flow (multi-step) with fallback logic
     * @param {string} downloadURL - Initial download link
     * @param {AbortSignal} signal - Abort signal
     * @param {Function} onProgress - Progress callback (optional)
     * @returns {Promise<Object>} - Final download button data
    ...
    async handleOneClickDownload(downloadURL, signal = null, onProgress = null) {
        try {
            ...
            // Step 1: Initial Page -> Find Entry Point
            const page1HTML = await this.fetchHTML(downloadURL, false, signal);
            const page1Doc = this.parseHTML(page1HTML);

            // Group keywords by priority (USER REQUEST: FASTCLOUD FIRST)
            const primaryKeywords = [
                'FASTCLOUD HIGH-SPEED', 'FASTCLOUD', 'HIGH SPEED DOWNLOAD',
                'GDRIVE', 'DIRECT', 'FAST DOWNLOAD'
            ];
            const secondaryKeywords = [
                'ONE CLICK DOWNLOAD', 'DIRECT DOWNLOAD', 'DOWNLOAD NOW',
                'DOWNLOAD', 'CLICK TO DOWNLOAD', 'GET LINK'
            ];

            // Helper to try keywords and then process
            const tryKeywords = async (keywords, isPrimary = true) => {
                const label = isPrimary ? 'FASTCLOUD' : 'ONE CLICK DOWNLOAD';
                if (onProgress) onProgress({ step: 1, message: `🚀 Attempting ${label}...` });

                let entryPointBtn = null;
                const tags = ['a', 'button'];

                // Try keywords with robust matching
                for (const tag of tags) {
                    for (const kw of keywords) {
                        entryPointBtn = this.findElementByText(page1Doc, tag, kw);
                        if (entryPointBtn) break;
                    }
                    if (entryPointBtn) break;
                }

                // Generic fallback if primary (FastCloud) fails to find a specific button
                if (!entryPointBtn && isPrimary) {
                    // Look for anything that looks like a big download button
                    entryPointBtn = page1Doc.querySelector('.entry-content a.btn, #main a.btn, .big-cta a, a.btn-lg, button.btn-lg');
                }

                if (!entryPointBtn) return null;

                const nextURL = this.resolveRelativeURL(entryPointBtn.getAttribute('href'));
                if (onProgress) onProgress({ step: 2, message: `Analyzing ${label} link...` });

                try {
                    const page2HTML = await this.fetchHTML(nextURL, false, signal);
                    const page2Doc = this.parseHTML(page2HTML);

                    let intermediateLink = page2Doc.querySelector('#download');
                    if (!intermediateLink) {
                        intermediateLink = this.findElementByText(page2Doc, 'a', 'Open Direct Download Link') ||
                            this.findElementByText(page2Doc, 'a', 'Continue to Download') ||
                            this.findElementByText(page2Doc, 'a', 'Download Now');
                    }

                    if (!intermediateLink) {
                        throw new Error('Download link (Page 2) not found');
                    }

                    return this.resolveRelativeURL(intermediateLink.getAttribute('href'));
                } catch (e) {
                    if (this.debug) console.warn(`[ScraperEngine] Attempt failed for ${label}:`, e.message);
                    return null;
                }
            };

            // TRY PRIMARY (FASTCLOUD)
            let finalPageURL = await tryKeywords(primaryKeywords, true);

            // FALLBACK TO SECONDARY (ONE CLICK) IF FAILED
            if (!finalPageURL) {
                if (this.debug) console.log('[ScraperEngine] FASTCLOUD failed, attempting ONE CLICK DOWNLOAD fallback...');
                finalPageURL = await tryKeywords(secondaryKeywords, false);
            }

            if (!finalPageURL) {
                throw new Error('Could not find download link after multiple attempts. আমাদের সাথে যোগাযোগ করুন (Contact us).');
            }

            // Step 3: Final Page -> Extract links
            ...
            const ctaContainer = page3Doc.querySelector('div[class*="big-cta"]') ||
                ...
                page3Doc.querySelector('.entry-content');
 ...
 */
    /**
     * Find element by text content (robust matching)
     * @param {Document} doc - Parsed HTML document
     * @param {string} tagName - HTML tag name (e.g., 'a', 'div')
     * @param {string} text - Text to search for
     * @returns {Element|null} - Found element or null
    ...
    findElementByText(doc, tagName, text) {
        ...
        const searchNormalized = this.normalizeText(text);

        for (const el of elements) {
            const elNormalized = this.normalizeText(el.textContent);
            ...
            if (title && this.normalizeText(title).includes(searchNormalized)) {
                return el;
            }
        }
        return null;
    ...
    /**
     * Normalize text for robust matching (removes emojis, extra whitespace, special chars)
    ...
    normalizeText(text) {
        ...
        return text.toLowerCase()
            ...
            .replace(/[^\w\s\u0980-\u09FF]/g, ' ') // Keep alphanumeric, whitespace, and Bengali

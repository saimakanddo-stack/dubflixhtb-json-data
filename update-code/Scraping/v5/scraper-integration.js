...
class ScraperIntegration {
        ...
        try {
            ...
            if (!result.success && !signal.aborted) {
                ...
                if (!container || !container.querySelector('.download-server')) {
                    this.clearLoading(); // Ensure loader is gone
                ...
                const container = document.getElementById('downloadOptions');
                if (container) {
                    const hasOptions = container.querySelector('.download-server');
                    if (!hasOptions) {
                        this.clearLoading();
                        if (result.downloadOptions && result.downloadOptions.length === 0) {
                            this.injectError("No download options found.");
                        } else {
                            // This case shouldn't happen if scraper emitted cards, but for safety
                            container.innerHTML = '';
                        }
                    } else {
                        // Skeletons might still be there if they weren't the first option or if logic missed them
                        this.clearLoading(); 
                    }
                }
            }
    ...
    appendDownloadOption(card) {
        ...
        // Ensure skeletons/loaders are gone before appending
        this.clearLoading();
    ...
    /**
     * Clear all types of loading indicators (legacy loaders and new skeletons)
     */
    clearLoading() {
        const container = document.getElementById('downloadOptions');
        if (!container) return;

        const loaders = container.querySelectorAll('.loading, .skeleton-options, .scraped-skeleton');
        if (loaders.length > 0) {
            container.innerHTML = '';
        } else if (container.textContent.toLowerCase().includes('loading')) {
            // Final fallback for plain text loaders
            container.innerHTML = '';
        }
    }
    ...
    async fetchAndPopulateLanguages() {
        try {
            ...
            if (!fetchUrl) {
                ...
                if (langList) langList.innerHTML = '<div class="sidebar-empty">No languages found.</div>';
                return;
            }
            ...
            if (!html || html.length < 100) {
                ...
                const langList = document.getElementById('languagesList');
                if (langList) langList.innerHTML = '<div class="sidebar-empty">Failed to load languages.</div>';
                return;
            }
            ...
            const langList = document.getElementById('languagesList');
            if (langList) langList.innerHTML = '<div class="sidebar-empty">Error loading languages.</div>';
            window.scrapedLanguages = [];
            if (typeof renderSidebarLanguages === 'function') renderSidebarLanguages();
        }
    }
    ...
    async fetchAndPopulateGenres() {
        try {
            ...
            if (!fetchUrl) {
                const genresList = document.getElementById('categoriesList');
                if (genresList) genresList.innerHTML = '<div class="sidebar-empty">No categories found.</div>';
                return;
            }
            ...
            if (!html || html.length < 100) {
                if (!cachedGenres) {
                    const genresList = document.getElementById('categoriesList');
                    if (genresList) genresList.innerHTML = '<div class="sidebar-empty">Failed to load categories.</div>';
                }
                return;
            }
            ...
            if (!localStorage.getItem('scrapedGenres')) {
                const genresList = document.getElementById('categoriesList');
                if (genresList) genresList.innerHTML = '<div class="sidebar-empty">Error loading categories.</div>';
            }
        }
    }
    ...
    async fetchAndPopulateTopCategories() {
        try {
            ...
            if (!fetchUrl) {
                ...
                if (topCatsList) topCatsList.innerHTML = '<div class="sidebar-empty">No categories found.</div>';
                return;
            }
            ...
            if (!html || html.length < 100) {
                ...
                const topCatsList = document.getElementById('topCategoriesList');
                if (topCatsList) topCatsList.innerHTML = '<div class="sidebar-empty">Failed to load categories.</div>';
                return;
            }
            ...
            const topCatsList = document.getElementById('topCategoriesList');
            if (topCatsList) topCatsList.innerHTML = '<div class="sidebar-empty">Error loading categories.</div>';
        }
    }
        ...
        try {
            ...
            if (cards.length === 0) {
                ...
                if (typeof showAllMoviesLoaded === 'function') {
                    const isFirstPage = this.cardScrapingState.currentPage === 1;
                    showAllMoviesLoaded(isFirstPage);
                }
                return;
            }

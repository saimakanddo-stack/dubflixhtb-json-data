...
class ScraperCardExtractor {
     ...
     * @param {ScraperEngine} scraperEngine - ScraperEngine instance for pagination extraction
     * @returns {Object} - { movieCards: Array, pagination: Object }
     */
    extractMovieCards(html, scraperEngine = null) {
        try {
            ...
            // Extract pagination if scraperEngine is provided
            let paginationData = null;
            if (scraperEngine && typeof scraperEngine.scrapePagination === 'function') {
                paginationData = scraperEngine.scrapePagination(doc);
            }

            return {
                movieCards: extractedCards,
    ...
    parseMovieCard(cardElement) {
        try {
            const cardData = {
                // Required fields
                post_url: '',
                share_Id: '', // Shareable ID extracted from post_url (format: category-id)
                title: '',
                imageUrl: '',
            ...
            if (imageLink) {
                ...
                cardData.share_Id = this.extractShareId(cardData.post_url); // Extract share_Id
            }
    ...
    /**
     * Extract share_Id from post_url
     * Format: category-id (e.g., "movie-abc123")
     * @param {string} postUrl - Full post URL
     * @returns {string} - Share ID in format "category-id"
     */
    extractShareId(postUrl) {
        if (!postUrl) return '';
        ...
        try {
            ...
            // Expected format: /category/id
            if (pathParts.length >= 2) {
                const category = pathParts[0]; // e.g., "movie"
                const id = pathParts[1]; // e.g., "efWhIVZPGW1jUBQabPMb95BLnQEEg..."
                return `${category}-${id}`;
            }

            return '';
        } catch (error) {
            console.error('[ScraperCardExtractor] Error extracting share_Id:', error);
            return '';
        }
    }

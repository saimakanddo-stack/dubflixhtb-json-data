...
class ScraperIntegration {
    ...
    showInlineLoading() {
        ...
        if (container) {
            container.innerHTML = `
                <div class="skeleton-options p-4">
                    <div class="skeleton-title skeleton mb-4" style="width: 50%;"></div>
                    <div class="skeleton-card skeleton mb-3" style="height: 100px;"></div>
                    <div class="skeleton-card skeleton mb-3" style="height: 100px;"></div>
                    <div class="skeleton-meta skeleton" style="width: 30%;"></div>
                </div>
            `;
        }
    }
    ...
    async handleCategoryFilter(name, url, type = 'category') {
        ...
        // Auto-expand relevant sidebar section
        if (type === 'top-category') {
            const topCatsHeader = document.getElementById('topCategoriesHeader');
            if (topCatsHeader) topCatsHeader.classList.add('active');
        } else if (type === 'language') {
            const langHeader = document.getElementById('languagesHeader');
            if (langHeader) langHeader.classList.add('active');
        } else if (type === 'category') {
            const catsHeader = document.getElementById('categoriesHeader');
            if (catsHeader) catsHeader.classList.add('active');
        }
    ...
    async continueCardScraping() {
        ...
        try {
            ...
            // RACE CONDITION FIX: Check if we are still active/not paused
            // If stopped while fetching, do not render or continue
            if (!this.cardScrapingState.isActive || this.cardScrapingState.isPaused) {
                if (this.config.debug) console.log('[ScraperIntegration] Scraping stopped/paused during fetch, aborting render');
                return;
            }
    ...
    async handlePageClick(button) {
        ...
        if (this.scrapedPages.has(pageNumber)) {
            // Auto-scroll to the first card of this page
            const firstCardId = this.scrapedPages.get(pageNumber);
            const firstCard = document.getElementById(firstCardId);

            if (firstCard) {
                firstCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
                console.log(`[ScraperIntegration] Scrolled to page ${pageNumber}`);

                // Highlight the card briefly
                firstCard.classList.add('highlight-flash');
                setTimeout(() => firstCard.classList.remove('highlight-flash'), 2000);
            }
            return;
        }
        ...
        const moviesGrid = document.getElementById('moviesGrid');
        if (moviesGrid) {
            // Add a small delay for skeletons to appear if it's too fast? 
            // Better to show immediately
            this.showSkeletonGrid(moviesGrid, 15); // Show 15 skeletons for the new page
        }
        ...
        try {
            ...
            // Remember the first card ID for this page
            const firstCardId = `scraped-card-${pageNumber}-0`;
            this.scrapedPages.set(pageNumber, firstCardId);

            // Hide skeletons before injecting real cards
            this.hideSkeletons(moviesGrid);

            // Inject movie cards below existing cards
            ...
            // Mark button as scraped
            button.classList.add('scraped');
            button.innerHTML = originalText;
            button.disabled = false;

            // Scroll to first card of the newly scraped page
            const firstCard = document.getElementById(firstCardId);
            if (firstCard) {
                setTimeout(() => {
                    firstCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
